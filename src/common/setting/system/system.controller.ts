import { Controller, Get, UseGuards, Res, Req, Query, HttpException, HttpStatus, Param, Headers, Post, UsePipes, ValidationPipe, Body, Put, Delete } from '@nestjs/common';
...
import { SettingDTO } from 'src/common/setting/dto/setting.dto';
import { ResponseController } from 'src/shared/response/response.controller';

@ApiTags('Setting: System')
@Controller('settingSystem')
export class SystemController {
  constructor(
    private readonly mainServices: SystemService,
  ...
  async paginate1(@Res() res, @Req() req, @Query() query, @Param() param, @User() payload, @Headers() header) {
    try {
      this.resdata.logData({ id: payload.id, user: payload, data: param });
      const lang = (typeof header.lang !== 'undefined' ? header.lang : '').toUpperCase();
  ...
  async createRoom(@Res() res, @User() payload, @Req() req, @Body() body: SettingDTO) {
  ...
  @UseGuards(new AuthGaurd())
  @UsePipes(new ValidationPipe())
  async update(@Res() res, @Req() req, @Param('id') id, @User() payload, @Body() body: SettingDTO) {
