...
import { Languages } from "./md-language.entity";

@Entity({ name: 'conf_settings' })
export class ConfSettings {
  ...
  async totalRow() {
    const total = await getManager().getRepository(ConfSettings).count({ isDelete: false });
    return total;
  }
}